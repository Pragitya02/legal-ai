    ...
    try {
        ...
        if (
            ...
            return res.status(403).json({
                success: false,
                message:
                    "Only citizen accounts can send consultation requests."
            });
        }
        ...
        if (advocateRows.length === 0) {
            return res.status(404).json({
                success: false,
                message:
                    "Selected advocate was not found."
            });
        }
        ...
        if (lawyerRows.length === 0) {
            return res.status(404).json({
                success: false,
                message:
                    "Advocate availability profile was not found."
            });
        }
        ...
        if (
            ...
            Number(
                scheduleRows[0].is_available
            ) !== 1
        ) {
            return res.status(409).json({
                success: false,
                message:
                    "The advocate is not available on the selected day."
            });
        }
        ...
        const scheduleStart =
            timeToMinutes(
                scheduleRows[0].start_time
            );

        const scheduleEnd =
            timeToMinutes(
                scheduleRows[0].end_time
            );
        ...
        if (
            ...
            return res.status(409).json({
                success: false,
                message:
                    "The selected time is outside the advocate's working hours."
            });
        }
        ...
        if (
            ...
            return res.status(409).json({
                success: false,
                message:
                    "This appointment slot already has a pending or confirmed request."
            });
        }
        ...
        return res.status(201).json({
            success: true,
            message:
                "Consultation request sent to the advocate.",
            appointment: {
                id: appointmentId,
                citizenId,
                advocateId:
        ...
        console.error(
            "CREATE CONSULTATION REQUEST ERROR:",
            error
        );
...
router.patch(
    ...
    async (req, res) => {
        try {
            ...
            // -------------------------------------------------
            // ADVOCATE ROLE VALIDATION
            // The application may represent advocates as
            // either "lawyer" or "advocate".
            // -------------------------------------------------
            ...
            if (!isAdvocate) {
                return res.status(403).json({
                    success: false,
                    message:
                        "Only advocate accounts can respond to consultation requests."
                });
            }
            ...
            if (
                ...
                return res.status(409).json({
                    success: false,
                    message:
                        "This consultation request has already been handled."
                });
            }
            ...
            if (action === "accept") {
                ...
                if (
                    ...
                    return res.status(409).json({
                        success: false,
                        message:
                            "This request could not be accepted because it was already handled."
                    });
                }
                ...
                if (
                    ...
                    try {
                        meeting =
                            await createMeetingForAppointment(
                                appointmentId
                            );
                    } catch (
                        meetingError
                        ...
                        console.error(
                            "CREATE MEETING AFTER ACCEPT ERROR:",
                            meetingError
                        );

                        // The appointment remains confirmed.
                        // The meeting can be created later
            ...
            // The existing Railway appointments enum does
            // not contain "declined".
            // We preserve the existing schema by using
            // "cancelled" plus a DECLINED marker.
            ...
            const [result] =
                await db.query(
                    ...
                    SET status = 'cancelled',
                        notes = CONCAT(
                            COALESCE(notes, ''),
                            '; DECLINED: advocate declined request'
                        )
                    WHERE id = ?
                      AND lawyer_id = ?
                      AND status = 'pending'
            ...
            if (
                ...
                return res.status(409).json({
                    success: false,
                    message:
                        "This request could not be declined because it was already handled."
                });
            }
            ...
            console.error(
                "RESPOND TO CONSULTATION REQUEST ERROR:",
                error
            );
